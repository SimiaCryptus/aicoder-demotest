...
class CommandAutofixActionTest : BaseActionTest() {
    ...
    fun testCommandAutofixAction() = with(remoteRobot) {
        TestUtil.speak("This demo showcases the Command Autofix feature, which automatically identifies and fixes issues across the codebase.")
        log.info("Starting testCommandAutofixAction")
        sleep(2000)

        step("Open project view") {
            TestUtil.speak("Opening the project view to access the project structure.")
            find(CommonContainerFixture::class.java, byXpath("//div[@class='ProjectViewTree']")).click()
            log.info("Project view opened")
        }

        step("Select a directory") {
            TestUtil.speak("Selecting a directory to apply Command Autofix.")
        ...
        step("Select 'AI Coder' menu") {
            TestUtil.speak("Accessing the AI Coder menu.")
        ...
        step("Click 'Auto-Fix' action") {
            TestUtil.speak("Selecting the 'Auto-Fix' action.")
            waitFor(Duration.ofSeconds(10)) {
                try {
                    ...
                    TestUtil.speak("Failed to find Auto-Fix action. Retrying.")
                    false
                }
            }
        }
        ...
        step("Configure Command Autofix") {
            TestUtil.speak("Configuring Command Autofix settings.")
            waitFor(Duration.ofSeconds(10)) {
        ...
        step("Interact with Command Autofix interface") {
            ...
            if (url != null) {
                ...
                TestUtil.speak("Command Autofix interface opened in a new window.")
                ...
                while (attempt <= 5) {
                    ...
                    try {
                        blockUntilDone(wait)
                        TestUtil.speak("Command Autofix operation completed.")
                        ...
                        TestUtil.speak("Command Autofix operation completed successfully. Build remains successful.")
                        break
                    } catch (e: Exception) {
                        attempt++
                        log.warn("Error interacting with Command Autofix interface", e)
                        blockUntilDone(wait)
                        TestUtil.speak("Error encountered. Retrying.")
                        ...
                        TestUtil.speak("Refreshing Command Autofix interface.")
                ...
                TestUtil.speak("Error retrieving Command Autofix interface URL.")
            }
            TestUtil.clearMessageBuffer()
        }

        TestUtil.speak("Demo concluded. Command Autofix feature demonstrated.")
    }
