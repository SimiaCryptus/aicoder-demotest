...
class CodeChatActionTest : BaseActionTest() {
    ...
    fun testCodeChatAction() = with(remoteRobot) {
        TestUtil.speak("Welcome to the AI Coder demo. We'll explore the Code Chat feature, which enables AI interaction for code-related queries and assistance.")
        ...
        step("Open a Kotlin file") {
            TestUtil.speak("Opening a Kotlin file for the Code Chat demonstration.")
        ...
        step("Select code") {
            TestUtil.speak("Selecting code to provide context for the AI.")
        ...
        step("Click 'Code Chat' action") {
            TestUtil.speak("Initiating the 'Code Chat' feature for an interactive dialogue with AI about our code.")
            waitFor(Duration.ofSeconds(15)) {
                try {
        ...
        step("Get URL from UDP messages") {
            ...
            if (url != null) {
                ...
                chatInput.click()
                TestUtil.speak("Entering a request for the AI to create a user manual for our class.")
                ...
                TestUtil.speak("Submitting the request to the AI.")
                log.info("Submitting request to AI")
                submitButton.click()
                Thread.sleep(3000) // Short pause after clicking submit
                TestUtil.speak("Waiting for the AI to process the request and generate a response.")
                try {
                    val markdownTab =
                    ...
                    Thread.sleep(2000)
                    TestUtil.speak("Switching to the Markdown tab for better readability of the AI response.")
                    markdownTab.click()
                    Thread.sleep(2000)
                    ...
                    TestUtil.speak("AI response is delayed. Skipping the copy action.")
                }

                Thread.sleep(3000) // Wait for the hide action to complete
                TestUtil.speak("We've successfully interacted with the Code Chat interface. As you can see, this feature provides quick, context-aware assistance for your coding tasks.")
                this@CodeChatActionTest.driver.quit()
            } else {
                log.error("No URL found in UDP messages")
                TestUtil.speak("Error: Unable to retrieve the necessary URL.")
            }
            TestUtil.clearMessageBuffer()
        }

        TestUtil.speak("Demo concluded. We've demonstrated initiating a Code Chat session, submitting a query, and receiving an AI-generated response.")
